...
import numpy as np
from tests.test_code_external.graph_reducer import szemeredi_lemma_builder as slb
...
class Codec:
    ...
    def __init__(self, e1, e2, samples):
        ...
        self.drop_edges_between_irregular_pairs = False
    ...
    def reduced_matrix(self, G, k, epsilon, classes, regularity_list):
        ...
        for r in range(2, k + 1):
            ...
            for s in (range(1, r) if not self.drop_edges_between_irregular_pairs else regularity_list[r - 2]):
                ...
                # print(f'r: {r-1}, s: {s-1}, bip dens: {bip_density}')
                reduced_sim_mat[r - 1, s - 1] = reduced_sim_mat[s - 1, r - 1] = bip_density
                if reduced_sim_mat[r-1, s-1] == 0.:
                    print(f'r: {r-1}, s: {s-1}, bip dens: {bip_density}')

        idx_zero = np.where(reduced_sim_mat == 0.)
